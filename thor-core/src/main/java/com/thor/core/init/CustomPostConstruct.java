...
import com.thor.sdk.common.exception.RTEception;
...
public class CustomPostConstruct {
    ...
    public void loadSuperTenantFromDB() {
        ...
        if(StringUtils.isBlank(id)){
            throw new RTEception(ResultStatus.INTERNAL_SERVER_ERROR);  //必须配置超管租户的租户编号
        }
        Tenant tenant = tenantService.getTenant(new TenantGetParam(id));
        if(tenant == null){
            throw new RTEception(ResultStatus.INTERNAL_SERVER_ERROR);  //如果使用多租户，则数据库必须配置超管租户
        }
