...
public class JWTFilter extends BasicHttpAuthenticationFilter {
    ...
    protected boolean preHandle(ServletRequest request, ServletResponse response) throws Exception {
        if((WebUtils.toHttp(request).getHeader(CommonConstants.token) == null)){
            return printError(response, ResultStatus.NO_ACCESS_TOKEN_EXCEPTION);
        }
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) {
        try{
            ...
            return printError(response, ResultStatus.JWT_INCORRECT_EXCEPTION);
        }catch (JWTExpiredException e){
            log.error(e.getMessage(), e);
            return printError(response, ResultStatus.SECURITY_INVALID_TOKEN);
        }
        return true;
    }
     ...
     * @return
     */
    private boolean printError(ServletResponse response, ResultStatus resultStatus) {
        try {
            response.setContentType(MediaType.APPLICATION_JSON_UTF8_VALUE);
            CommonResult result = CommonResult.put(resultStatus);
