  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="page.status" :placeholder="$t('monitor.columns.status')" multiple clearable collapse-tags class="filter-item" size="mini" style="width: 160px">
        <el-option v-for="item in doneJobInstanceStatuses" :key="item.value" :label="item.label" :value="item.value" />
      </el-select>
...
<script>
import { listReq } from '@/api/monitor/history'
...
export default {
  ...
  methods: {
    ...
    formatStatus(item) {
      for (let i = 0; i < this.doneJobInstanceStatuses.length; i++) {
        const option = this.doneJobInstanceStatuses[i]
